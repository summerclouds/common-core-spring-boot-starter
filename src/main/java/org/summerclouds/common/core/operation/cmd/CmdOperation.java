...
public abstract class CmdOperation extends AbstractOperation {
    ...
    public boolean hasAccess(TaskContext context) {
        return MSecurity.hasPermission(
                CmdOperation.class, MSecurity.EXECUTE, getClass().getCanonicalName());
    }
    ...
    protected OperationResult execute(TaskContext context) throws Exception {
        ...
        PojoModel model =
                new PojoParser()
                        .parse(getClass(), "_", CmdOption.class, CmdArgument.class)
                        .filter(true, true, false, true, false)
                        .getModel();
        ...
        } else context.getParameters().remove(PARAMETER_INPUT_STREAM);
            ...
            try (ICloseable ioEnv = MSystem.useIO(os, os, is)) {
                try (ICloseable logEnv = ThreadConsoleLogAppender.sendTo(os)) {
                    ...
                    if (consoleSize != null) {
                        ...
                        if (parts.length > 1) console.setHeight(MCast.toint(parts[1], 24));
                    }
    ...
    protected DefRoot createDescriptionForm() {
        ...
        for (PojoAttribute<?> attr : model) {
            ...
            if (optionA != null) {
                ...
                if (optionA.required()) options.add(new FaMandatory());
            } else {
                name = "" + argumentA.index();
                title = "argument " + attr.getName();
                description = argumentA.description();
                if (argumentA.required()) options.add(new FaMandatory());
            }
            if (attr.getManagedClass()
                    .isArray() /*|| Collection.class.isAssignableFrom(attr.getManagedClass())*/) {
            ...
            if (attr.getManagedClass().isEnum()) {
                options.add(
                        new DefAttribute(
                                "values",
                                Arrays.toString(attr.getManagedClass().getEnumConstants())));
            }
            ...
            else if (attr.getManagedClass() == Integer.class || attr.getManagedClass() == int.class)
                def = new FmNumber(name, FmNumber.TYPES.INTEGER, title, description, optionsArray);
            else if (attr.getManagedClass() == Double.class
                    || attr.getManagedClass() == double.class)
                def = new FmNumber(name, FmNumber.TYPES.INTEGER, title, description, optionsArray);
            else if (attr.getManagedClass() == Float.class || attr.getManagedClass() == float.class)
                def = new FmNumber(name, FmNumber.TYPES.FLOAT, title, description, optionsArray);
            else if (attr.getManagedClass() == Boolean.class
                    || attr.getManagedClass() == boolean.class)
                def = new FmCheckbox(name, title, description, optionsArray);
            else if (attr.getManagedClass().isEnum())
                def = new FmOptions(name, title, description, optionsArray);
            else def = new FmText(name, title, description, optionsArray);
