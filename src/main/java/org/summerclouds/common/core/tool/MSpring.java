...
public class MSpring {
    ...
    public static <T> T lookup(Class<T> class1) {
        ...
        try {
            ...
            if (PlainValues.getBoolean("sping.lookup.debug", false)) e.printStackTrace();
            if (isStarted()) noLookup.add(class1.getCanonicalName());
            return (T) getDefaultActivator().getObject(class1.getCanonicalName());
    ...
    public static IProperties getAllValues() {
        ...
        if (environment != null) {
            MutablePropertySources propSrcs =
            ...
            StreamSupport.stream(System.getProperties().entrySet().spliterator(), false)
                    ...
                    .forEach(
                            pair ->
                                    props.put(
                                            String.valueOf(pair.getKey()).substring(4),
                                            pair.getValue()));
        }
        return props;
    }
    ...
    public static void setStatus(STATUS status) {
        ...
        if (status == STATUS.CLOSED) noLookup.clear();
    }
    ...
    public static <T> List<Class<? extends T>> findAnnotatedClasses(
            Class<? extends Annotation> annotationType, boolean checkConditions) {
    ...
    public static <T> List<Class<? extends T>> findAnnotatedClasses(
        ...
        ClassPathScanningCandidateComponentProvider provider =
                createComponentScanner(annotationType);
    ...
    public static String getMainPackages() {
        if (mainPackage == null) {
            ...
            Class<?> clazz =
                    candidates.isEmpty() ? null : candidates.values().iterator().next().getClass();
            if (clazz != null) {
                ...
                if (componentScan != null) {
                    ...
                    for (Class<?> item : componentScan.basePackageClasses())
                ...
                if (springBootApplication != null) {
                    ...
                    for (Class<?> item : springBootApplication.scanBasePackageClasses())
                        list.add(item.getPackageName());
                }
                if (list.size() > 0) mainPackage = MString.join(list, ",");
                else mainPackage = clazz.getPackageName();
            }
        }
        return mainPackage;
    }
